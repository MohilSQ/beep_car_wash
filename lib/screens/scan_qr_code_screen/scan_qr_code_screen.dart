...
class ScanQRCodeScreen extends GetView<ScanQrCodeController> {
  ...
  const ScanQRCodeScreen({Key? key, this.screenName}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    controller.screen.value = screenName ?? "";
    return GetBuilder<ScanQrCodeController>(builder: (logic) {
      return Scaffold(
        body: SafeArea(
          top: false,
          bottom: false,
                  ...
                  CustomButton(
                    onPressed: () {
                      if (controller.code.text.trim().isEmpty) {
                        controller.utils.showToast(message: "Please enter machine code", context: context);
                      } else {
                        Get.back(result: controller.code.text.trim());
                      }
                    },
